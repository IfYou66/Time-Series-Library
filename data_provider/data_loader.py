...
#     ]
#     # ================== 反泄漏/稳定性策略（可按需关闭/修改） ==================
#     APPLY_INSTANCE_NORM = True   # 改为：每文件“仅用训练窗口”估计统计量；val/test 复用同一统计
#     APPLY_DIFF          = True   # 一阶差分（首帧补0）
#     VAL_TEST_GAP        = 0.05   # train 与 val/test 之间留出 5% gap
#     EPS                 = 1e-5   # 数值稳定
#     # ======================================================================
#
...
#         self.feat_dim = len(keep_names)
#
#         # 读取文件（仅一次），先不做实例归一化，避免泄漏；可先做差分
#         self.file_arrays = []      # [(T,C), label, (n_head, gap, n_val, n_test)]
#         self.windows_per_file = []
#         self.data_proc = []        # 存放“仅做了 DIFF 的数据”（避免把 val/test 统计泄入）
#         total_files = 0
#
#         for p, lbl in self.class_files:
#             if not os.path.isfile(p):
...
#             # 截整窗
#             x = x[:n_all * self.seq_len, :]
#             self.data_proc.append(x)  # 只做了 DIFF 的版本
#             self.file_arrays.append((x, lbl, (n_head, gap, n_val, n_test)))
#             self.windows_per_file.append(n_all)
#             total_files += 1
#
...
#             base += n_all
#
#         # ------------------ 关键修复：每文件实例归一化 = 仅用该文件的训练窗口统计 ------------------
#         self.file_train_stats = None
#         if self.APPLY_INSTANCE_NORM:
#             self.file_train_stats = {}  # f_idx -> (mean[C], std[C])
#             F = len(self.data_proc)
#             for f_idx in range(F):
#                 segs = []
#                 for g in self.indices_train:
#                     f, w = self.index_map[g]
#                     if f != f_idx:
#                         continue
#                     start = w * self.seq_len
#                     segs.append(self.data_proc[f_idx][start:start + self.seq_len])  # [L,C]
#                 if len(segs) == 0:
#                     # 退化：此文件无训练窗口时，使用 0/1（避免崩溃）
#                     mean = np.zeros((self.feat_dim,), dtype=np.float32)
#                     std  = np.ones((self.feat_dim,), dtype=np.float32)
#                 else:
#                     M = np.concatenate(segs, axis=0)  # [(#wins*L), C]
#                     mean = M.mean(axis=0).astype(np.float32)
#                     std  = (M.std(axis=0) + self.EPS).astype(np.float32)
#                 self.file_train_stats[f_idx] = (mean, std)
#         # ----------------------------------------------------------------------------------------
#
#         # 拟合 StandardScaler（仅用训练窗口；且在拟合前先做“与推理一致”的 InstanceNorm）
#         self.scaler = StandardScaler()
#         train_chunks = []
#         for g in self.indices_train:
#             f_idx, w = self.index_map[g]
#             start = w * self.seq_len
#             seg = self.data_proc[f_idx][start:start + self.seq_len].astype(np.float32)  # [L,C]
#             if self.APPLY_INSTANCE_NORM:
#                 mean, std = self.file_train_stats[f_idx]
#                 seg = (seg - mean[None, :]) / std[None, :]
#             train_chunks.append(seg)
#         train_mat = np.concatenate(train_chunks, axis=0)  # [N_train*L, C]
#         self.scaler.fit(train_mat)
#
...
#             return len(self.indices_test)
#
#     # 小工具：在任意阶段对片段应用“train-only”的实例归一化
#     def _apply_instance_norm(self, seg, f_idx):
#         if not self.APPLY_INSTANCE_NORM:
#             return seg
#         mean, std = self.file_train_stats[f_idx]
...
#
#         f_idx, w = self.index_map[g]
#         x_diff_only = self.data_proc[f_idx]             # 仅做了 DIFF 的数据
#         lbl, _split = self.file_arrays[f_idx][1], self.file_arrays[f_idx][2]
#
#         start = w * self.seq_len
#         seg = x_diff_only[start:start + self.seq_len]   # [L, C]
#
#         # 推理/训练统一的变换顺序：InstanceNorm(train-only) -> Global StandardScaler
#         seg = self._apply_instance_norm(seg, f_idx)
#         seg = self.scaler.transform(seg)
#
#         x_t = torch.from_numpy(seg).float()
#         y_t = torch.tensor(lbl, dtype=torch.long)
#         return x_t, y_t
