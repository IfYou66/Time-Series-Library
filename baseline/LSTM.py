...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.pooling  = getattr(args, 'lstm_pool', 'mean').lower()
    ...
    def forward(self, x, padding_mask=None, x_mark=None, y_mark=None):
        ...
        """
        lstm_out, _ = self.lstm(x)  # [B, L, H] 或 [B, L, 2H]（双向）

        if self.pooling == 'mean':
            feat = self._masked_mean(lstm_out, padding_mask)
        elif self.pooling == 'max':
            feat = self._masked_max(lstm_out, padding_mask)
        else:  # 'last'
            feat = self._gather_last(lstm_out, padding_mask)
